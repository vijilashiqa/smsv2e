...
import { CountryService, HeadendService, PackageService, PagerService, RoleservicesService, VendorService } from '../../_services';
...
@Component({
  selector: 'ngx-custboxassignlog',
  templateUrl: './custboxassignlog.component.html',
  styleUrls: ['./custboxassignlog.component.scss']
})
export class CustboxassignlogComponent implements OnInit {
  ...
  loading = false;
 branch = '';custid;
  model_opt = ''; head_opt = ''; address = '';
  pack: any = []; pack_type = ''; package = ''; from_cdate = ''; 
castype = ''; listsubscriberl; getcas; listsubscriber;
listsubid;
...
liststb;
  ...
  async subid() {
    this.listsubscriber = await this.stb.liststb({ boxid: this.boxparing })
    console.log('listsubscriber', this.listsubscriber)
  }
  async vcparing() {
